...
6. **Reference pricer fallback** (`IF THE ANCHOR GOES DOWN`) binds when a snapshot contains NO pinnacle quotes: `0` — EV detection devigs a LEAVE-ONE-OUT consensus benchmark (for each candidate book, the best odds per selection among the OTHER books ONLY — never the book's own price, so no self-referential edge — and the line must be COMPLETE: ≥ 2 selections and every selection in the group present among those other books, else nothing) and the recheck mirrors it; middles and arbs continue. [F1: the original "best odds across all books" was self-referential and was corrected to leave-one-out.] `1` — EV and MIDDLE detection pause, arbs continue. `2` — no candidates at all. In sim the anchor never goes down (the provider always quotes pinnacle) — the wiring is proven with a stub provider that omits pinnacle, never by faking an outage. The radios echo the verbatim labels; switching journals (Design §12).
...
and add below `detectEvs` — ⚠️ **F1 CORRECTION (SUPERSEDED):** the `detectEvsConsensus` block below (and the "best odds across ALL books" recheck helper, and the "consensus devig ... 1 EV" test earlier in this doc) is **degenerate/self-referential** — the best-priced book is measured against a benchmark built from its own price, so it trivially shows +EV. The SHIPPED implementation excludes the candidate book (**leave-one-out**) and requires a **complete line** (≥2 selections, all present among the ≠B books): see `server/src/pipeline/candidates.ts` `detectEvsConsensus`, `server/src/pipeline/verify.ts` `consensusFairProb`, and the leave-one-out tests in `candidates.test.ts` / `mixcap.test.ts`. Read the code below as intent only:
...
Run: `npm test -w server && npm run typecheck -w server`
Expected: full suite PASS (8 new tests; typecheck clean). [F1 CORRECTION — the earlier "defaults preserve old behavior" was FALSE: the default `anchorFallback: 0` DOES change pinnacle-less behavior (a no-anchor snapshot now yields consensus EV), so the six pinnacle-less ARB candidates tests are NOT untouched — they set `anchorFallback: 1` to stay ARB-scoped.]
